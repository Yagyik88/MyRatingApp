  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO users (name, email, address, password, role)
       VALUES ($1,$2,$3,$4,'normal') RETURNING id, role`,
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (!passRegex.test(newPassword))
      return res.status(400).json({ message: "New password must meet requirements" });
...
exports.getUser = async (req, res) => {
  try {
    ...
    if (stores.length === 0) {
      return res.json({ ...user, stores: [], average_rating: 0 });
    }
